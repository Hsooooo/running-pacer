...
- **인증 토큰 역직렬화 오류 수정**: OAuth2 서버의 JWT 클레임 내 `user_id`를 `Long`에서 `String`으로 변경하여 Jackson의 `InvalidTypeIdException` 문제를 해결했습니다.
- **인증 컨텍스트 유연화**: `McpUserContext`에서 `user_id` 클레임을 추출할 때 `Number`와 `String` 타입을 모두 지원하도록 개선했습니다.
- **인증 데이터 초기화**: 타입 불일치로 인한 런타임 에러 방지를 위해 기존 `oauth2_authorization` 데이터를 초기화하는 마이그레이션(V9)을 추가했습니다.
...
- **시스템 안정성**: 인증 서버의 데이터 직렬화 문제를 근본적으로 해결하여 로그인/인증 신뢰성 확보.
