...
class RunningAggregationService(
        ...
        private val workoutClassifier: WorkoutClassifier
) {
    ...
    fun refreshActivityDerivedAndDailyStats(userId: Long, activityId: Long) {
        ...
        val paceFromStream =
                streamsRepo.findByIdOrNull(activityId)?.let { streams ->
                ...
                paceFromStream
                        ?: run {
                            if (activity.movingTimeS > 0 && activity.distanceM > 0) {
                                (activity.movingTimeS.toDouble() / (activity.distanceM / 1000.0))
                                        .toInt()
                            } else null
                        }
        ...
        val statDate =
                activity.startTimeUtc.toLocalDate() // UTC 기준. 필요하면 user timezone 반영(Phase 1은 단순화)
